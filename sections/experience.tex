...
\newcommand{\plone}[1]{\href{https://web.archive.org/web/20191102091002/https://support.precisionlender.com/hc/en-us/articles/115009642807-Rate-Sheet-Printouts}{#1}}
	...
	{%
		\bodytext{%
			...
			\begin{tightemize}
				...
				\item Delivered frontend dashboard using \texttt{NextJS} and \texttt{Plotly}
				\item Enabled future project success by deploying documentation on \texttt{Azure Static Web Apps} and enabling continuous integration (CI) tooling using \texttt{Azure Pipelines}
			\end{tightemize}
		}
	}
}
...
{%
	...
	{%
		\bodytext{%
			Led the development of \foreshadow{Foreshadow}, an open-source Python automatic machine learning package.
			\begin{tightemize}
				\item Architected the system to build end-to-end ML pipelines that interop \texttt{sklearn} and \texttt{pandas} (100\textsmaller{\%} branch test coverage)
				\item Wrote an \gppaperone{extensible benchmarking} package and \gppapertwo{research paper}that gained \gppaperthree{significant Twitter traction}
...
{\headedsubsection{Software Engineer Intern}
	...
	{%
		\bodytext{%
			...
			\begin{tightemize}
				\item Architected and developed a scalable natural language processing (NLP) system that cached queries using a C\# Azure Service Fabric Stateful microservice in user by over 100 banks worldwide
			\end{tightemize}
		}
	}
}
